...
func (p TCountDistinctImplType) String() string {
  ...
  case TCountDistinctImplType_StdSet: return "StdSet"
  }
  return "<UNSET>"
}
...
func TCountDistinctImplTypeFromString(s string) (TCountDistinctImplType, error) {
  ...
  case "StdSet": return TCountDistinctImplType_StdSet, nil 
  }
...
var _elem6 int64
    ...
    p.SparseSet = append(p.SparseSet, _elem6)
  }
  ...
  if p.IsSetSparseSet() {
    ...
    for i := 0; i<len(p.SparseSet); i++ {
      for j := i+1; j<len(p.SparseSet); j++ {
        if reflect.DeepEqual(p.SparseSet[i],p.SparseSet[j]) { 
          return thrift.PrependError("", fmt.Errorf("%T error writing set field: slice is not unique", p.SparseSet[i]))
        }
      }
    }
